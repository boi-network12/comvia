...
// const socketInstanceCache = new Map<string, { socket: Socket | null; refCount: number }>();
...
//   // ✅ Use a unique key for this socket instance
//   const instanceKey = `${visitorIdRef.current}_${options.companyId || ''}`;
...
//    // ✅ Get or create cached socket instance
//   const getCachedSocket = useCallback(() => {
//     if (!socketInstanceCache.has(instanceKey)) {
//       socketInstanceCache.set(instanceKey, { socket: null, refCount: 0 });
//     }
//     return socketInstanceCache.get(instanceKey)!;
//   }, [instanceKey]);
...
//       // Decrease ref count
//       const cached = getCachedSocket();
//       cached.refCount--;
      
//       // Only disconnect if no more references
//       if (cached.refCount <= 0 && cached.socket) {
//         cached.socket.disconnect();
//         cached.socket = null;
//         socketInstanceCache.delete(instanceKey);
//         console.log('🔌 [Widget] Socket instance cleaned up');
//       }
...
// ✅ SINGLE GLOBAL SOCKET INSTANCE - Only one socket per visitor
let globalSocket: Socket | null = null;
let globalSocketConnected = false;
let globalSocketError: string | null = null;
...
export function useSocket(options: UseSocketOptions = {}): UseSocketReturn {
  ...
  const [error, setError] = useState<string | null>(globalSocketError);
  ...
  // ✅ Store callbacks to re-attach on reconnect
  ...
  // Cleanup on unmount - but DON'T disconnect if other components are using it
  useEffect(() => {
    mountedRef.current = true;
    
    return () => {
      mountedRef.current = false;
      // Don't disconnect here - let the global socket live
    };
  }, []);
  ...
  const connect = useCallback(() => {
    ...
    if (!globalListenersRegistered) {
      ...
      socket.on('connect_error', (err: Error) => {
        ...
        console.error('❌ [Widget] Connection error:', err.message);
        globalSocketError = err.message;
        setError(err.message);
        isConnectingRef.current = false;
        setReconnectAttempts(prev => {
          const newAttempts = prev + 1;
          globalReconnectAttempts = newAttempts;
          return newAttempts;
        });
      });
      ...
      socket.on('agent_message', (data: { content: string; conversationId: string; senderId: string }) => {
        if (!mountedRef.current) return;
        console.log('📨 [Widget] Agent message:', data);
        if (callbacksRef.current.onAgentMessage) callbacksRef.current.onAgentMessage(data);
      });

      socket.on('new_message', (message: any) => {
        if (!mountedRef.current) return;
        
        // ✅ Check if this is from the visitor themselves (skip)
        ...
        if (isAgent && callbacksRef.current.onAgentMessage) {
