...
export const Widget: React.FC = () => {
  ...
  // ✅ Only connect once when component mounts and settings are loaded
  useEffect(() => {
    // Only attempt connection if we haven't already, we're not loading, and we have a companyId
    if (!connectionAttempted.current && !isLoading && companyId) {
      connectionAttempted.current = true;
      console.log('🔌 Widget mounting, initiating connection...');
      connectSocket();
    }
  }, [isLoading, companyId, connectSocket]);

  // ✅ Also try to connect if connection drops and we have a companyId
  useEffect(() => {
    // If we're connected, no need to reconnect
    if (isConnected) return;
    
    // If we've already attempted connection and it failed, retry after delay
    if (connectionAttempted.current && companyId && !isLoading) {
  ...
  }, [isConnected, companyId, isLoading, connectSocket]);
